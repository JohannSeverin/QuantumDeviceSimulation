...
class System(ABC):
    ...
    def __init__(self):
        ...
        device_params = {
            key: self.devices[key].parameters for key in self.devices.keys()
        }
    ...
    def system_parameters_to_be_swept(self):
        ...
        self.should_be_swept = np.any(
            [
                isinstance(self.system_parameters[key], np.ndarray)
                for key in self.system_parameters.keys()
            ]
        )
        ...
        if self.should_be_swept:
            self.sweep_parameters = [
                key
                for key in self.system_parameters.keys()
                if isinstance(self.system_parameters[key], np.ndarray)
            ]
            self.sweep_list = {
                key: self.system_parameters[key] for key in self.sweep_parameters
            }
        else:
...
class DispersiveQubitResonatorSystem(System):
    ...
    def __init__(
        ...
        Instead of drive,
        """
        self.devices = {"qubit": qubit, "resonator": resonator}
